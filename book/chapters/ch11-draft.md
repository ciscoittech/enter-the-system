...
Notice the Content pipeline is 25-100x more expensive than a study quiz. That's not because content is harder. It's because the pipeline has more stages, each stage reads and writes text, and some stages pull in web search results that add thousands of tokens to the context. Power has a meter running.
...
![Token cost breakdown: startup cost versus per-action cost for quiz, cover letter, and pipeline operations](../diagrams/png/ch11-token-cost.png)

*Token cost breakdown — startup costs (CLAUDE.md + state + skills) versus per-action costs across different system operations.*
...
![Model ladder: fast, standard, and reasoning tiers with example tasks and relative cost ratios](../diagrams/png/ch11-model-ladder.png)

*The model ladder — three tiers with example tasks at each level and the cost spread between them.*
...
Your state file loads every session. The 50-row guideline from Chapter 5 isn't just about focus — it's about cost. A 20-row file is ~300 tokens. A 200-row file is 2,000-3,000. Over 100 sessions a month, that's 200,000 extra tokens for context Claude mostly ignores. Archive aggressively. Keep the working table tight.

### 2. Use hooks instead of model judgment for rule-based checks.

Hooks run on your machine. Zero tokens, zero dollars. The verify-cover-letter hook from Chapter 7 catches fabrication for $0.00 — asking the model to self-check costs $0.02-0.05 every time. If the check can be written as a grep, a word count, or a pattern match, make it a hook. Save the model for things that require understanding.

### 3. Load skills selectively.

A 400-line skill costs ~400 tokens every time it loads. Don't load your editorial-voice skill for a study quiz. Your CLAUDE.md controls this: add conditional notes like "Load editorial-voice only when drafting content." Skills that aren't imported cost nothing — they're just files on disk.

### 4. Cache connection results.

Web search results don't change every hour. Save them to a file and load the file next time. After your Job Hunting System searches for company news, tell Claude: "Save those results to `research/acme-corp-2026-06.md`." Next session, read the cached file instead of re-searching.

### 5. Break pipelines into separate sessions.

Running a pipeline interactively keeps the entire context window open — Stage 1's research text is still loaded during Stage 5's review. The alternative: run stages as separate sessions. Your state file tracks which stage you're on. Each stage loads a clean context, does its job, updates state, and exits. No bloated context windows.

### Impact at a glance

| Strategy | Before | After | Monthly Savings (100 sessions) |
|----------|--------|-------|-------------------------------|
| Lean state file (200→20 rows) | ~3,000 tokens/session | ~300 tokens/session | ~270K tokens |
| Hook vs. model check (per check) | $0.02-0.05/run | $0.00/run | $2-5 per check type |
| Selective skill loading | ~800 tokens/session | ~400 tokens/session | ~40K tokens |
| Cached web search | $0.05-0.15/search | $0.001/file read | $5-15 on repeated searches |
| Staged pipeline (5 stages) | ~12K tokens (full context) | ~8K tokens (per-stage) | ~400K tokens |
...
For reference, tools like Langfuse trace every API call with cost, latency, and token count. Per-tool breakdowns. Monthly budgets with alerts. That's enterprise-grade visibility.
...
**Your validation layer is always free.** Every shell script in your hooks folder runs on your machine. No tokens, no API calls, no cost. Every check you move from "ask the model" to "run a script" saves money. Word-count checks, banned-phrase scans, duplicate-application detectors. All free.
...
A production system processing thousands of data points might cost $50-100/month. That system informs purchasing decisions worth many times that amount. Your personal systems cost $5-20/month and save you hours every week. The return is obvious once you look at it honestly.
