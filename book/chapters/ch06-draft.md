...
```
1. You start a session → Claude reads CLAUDE.md (rules) and scans skill descriptions
2. You type your prompt → Claude matches it to a skill and loads editorial-voice
3. Claude reads content-state.md → drafts with your voice loaded, not guessing
```
...
The file format is simple. At the top of `SKILL.md`, you put a small header block between `---` lines (just a name and description). The three dashes at the top and bottom are called frontmatter — it's metadata that tells the tool what this file is for. Below that, the skill content in markdown. Same format you've been writing all along.
...
**This isn't a Claude Code feature.** This pattern works in Claude Code, Codex, Cursor, and most AI coding tools — they all have a way to load external knowledge files. The concept is the same everywhere: separate your instructions (what to do) from your expertise (how to do it well), and load the expertise when relevant. If you switch tools next year, your skill files transfer. Rename the folder, keep the content.
...
Each system gets a skill folder and a CLAUDE.md update. Same pattern as the Content System build above — a `SKILL.md` with frontmatter, real examples, and a CLAUDE.md that points to it.

| System | Skill name | What it contains | What changes |
|--------|-----------|-----------------|--------------|
| Study | `study-method` | How you learn (examples-first vs. theory-first, session length, quiz format, analogies from your field) | Quiz questions match how you actually learn. Claude teaches your way, not its default way. |
| Job Hunting | `career-profile` | Full work history, cover letters that got callbacks (annotated), target role criteria, industry keywords | Cover letters cite real achievements with real numbers. You never re-explain your background. |
| Project Mgmt | `pm-methodology` | Your process (agile, kanban, whatever you actually do), status update formats per audience, risk criteria, definition of done | Status updates match YOUR format — the one your boss actually reads, not a generic template. |

Each skill lives in `.claude/skills/[skill-name]/SKILL.md`. Each system's CLAUDE.md gets updated to reference its skill, just like you did for the Content System in Step 4 above. The structure is identical: frontmatter at the top (name + description), then your real examples and patterns below it.
