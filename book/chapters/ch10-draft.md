...
Your CLAUDE.md from Chapter 4 set a consistent personality. This system does the same thing, but with three personalities, each tuned for a different kind of work. The base prompt stays constant. The overlay changes based on mode.
...
Your skills from Chapter 6 gave Claude expertise in one domain at a time. This system's 37 tools give the agent expertise across finance, market intelligence, competitive analysis, field operations, inventory, social media trends, demand scoring, supply chain mapping, shipping logistics, and task management.
...
This is the duplicate detection problem from Chapter 5, where your state file prevented duplicate cover letters for the same company. The alias map resolves identities across thousands of data points instead of dozens of rows.
...
| Production feature | Book component (chapter) |
|---|---|
| 400+ line system prompt with mode detection | CLAUDE.md — persistent instruction (Ch 4) |
| MongoDB collections (22,000+ quotes, 274 vision analyses, 96 surveys) | State files — structured data that persists and grows (Ch 5) |
| 37 specialized tools across finance, supply chain, demand scoring | Skill files — externalized knowledge loaded on demand (Ch 6) |
| Minimum-overlap thresholds, price ratio gates, model-validated output | Hooks — automated checks that catch errors before they ship (Ch 7) |
| Trade channels, social media APIs, field notes, surveys, vision data, web search | Connections — external data sources feeding the system (Ch 8) |
| Data ingestion → entity normalization → aggregation → agent reasoning → validated recommendation | Pipeline — multi-stage workflow with quality gates (Ch 9) |
...
This production system has its own equivalent of your `.claude/` directory: a project folder with system prompts that load automatically, specialized tool definitions, quality gates registered in configuration, and workflow definitions that chain stages together. The folder structure that Chapter 9 called "your workflow engine" is exactly what powers this system.
...
**The system gets smarter.** Every new price quote, every new survey response, every field note from a market visit makes the supply chain map more accurate. The price correlation algorithm runs against an ever-growing dataset. Three months of data produces tentative supplier-reseller guesses. Six months produces confident maps. A year produces an intelligence asset no competitor has. That's the feedback loop from Chapter 5, running continuously across a dataset that never stops growing.
...
The scale comes not from learning new concepts, but from feeding more data into the concepts you already know.
