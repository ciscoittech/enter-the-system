...
This book got you building. It deliberately stayed at the systems level and skipped the theory underneath. Now that your systems are running and producing real output, here's where to go next to make them sharper.

**Prompt engineering.** You learned structured prompts in Chapter 4. The next level: chain-of-thought prompting (asking the AI to reason step by step before answering), few-shot learning (giving 2-3 examples of what you want in the prompt itself), and prompt chaining (breaking a complex request into sequential smaller prompts). These techniques improve every component you've built, especially skills and pipeline stages.

**How LLMs actually work.** Tokens, attention, temperature, context windows. You don't need this to build systems, but you need it to debug them at scale. When your system produces inconsistent output, understanding that the AI literally cannot "see" text that fell outside its context window explains the failure instantly. Andrej Karpathy's lectures and Anthropic's documentation are the best starting points.

**Model selection and routing.** Chapter 11 introduced the model ladder. Going deeper means understanding benchmark comparisons, cost-per-quality trade-offs, when open-source models (Llama, Mistral, Qwen) beat commercial ones for specific tasks, and how to set up automatic routing so your system picks the right model per stage without you deciding every time.

**Context window management.** Your state files, skills, and CLAUDE.md all consume context. As your systems grow, you'll need strategies for compression, summarization, chunking, and prioritizing what the AI sees. This is the difference between a system that works with 30 items in state and one that works with 3,000.

**Advanced agent architectures.** You built single-agent workflows with optional subagents. Production systems use orchestrator-worker patterns, evaluation loops, autonomous planning, and tool-use chains that run without human approval. Anthropic's Building Effective Agents guide (docs.anthropic.com/en/docs/build-with-claude/agentic-patterns) is the direct next step.

**Systems thinking foundations.** Everything in this book rests on systems thinking principles that predate AI by decades. Three sources will deepen your understanding of why the framework works:

- *Thinking in Systems* by Donella Meadows: stocks, flows, feedback loops, leverage points. The "why" behind every design pattern in this book. Her concept of system archetypes will help you spot patterns in your own workflows that this book didn't cover.
- *The Goal* by Eliyahu Goldratt: the Theory of Constraints. Find the bottleneck ("Herbie"), fix that one thing, ignore everything else until it's resolved. This is the debugging protocol from Chapter 12 and the pipeline optimization from Chapter 11 in their original, deeper form.
- *Enter the System* by John Danaher: systematic decomposition applied to jiu jitsu. Different domain, same architecture. Subsystems with clear entries, controls, and finishes. If the martial arts framing clicks for you, it'll reinforce every concept in this book.

Short on time? Search YouTube for "Goldratt Theory of Constraints explained" and "Donella Meadows systems thinking." Ten-minute overviews that will land differently now that you've built the systems yourself.

These topics won't change what your systems do. They'll change how well they do it. Start with whichever one addresses your biggest current bottleneck. If your systems produce inconsistent output, study prompt engineering. If they're slow or expensive, study model selection. If they break at scale, study context management. Let the system tell you what to learn next.
