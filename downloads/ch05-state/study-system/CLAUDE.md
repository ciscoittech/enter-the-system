...
At the end of every session, update `study-state.md`:
- Add the session to the Session Log with date, topics, score
- Update the Topics table with new scores and mastery levels
- Recalculate Weak Areas (any topic scored < 70% more than once)
- Update Next Session Focus based on current weak areas

Never delete rows from the Topics table — they track progress over time.
